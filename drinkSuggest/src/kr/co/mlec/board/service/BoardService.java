...
public class BoardService {
	...
	public void insertReplyBoard(BoardVO board, List<BoardFileVO> fileList) {
		...
		int boardNo = boardDao.selectBoardNo();// num tilte write 다 들어있어서 boardNo 가져옴
		int groupNo = boardDao.selectCurrentGroupNo(board.getComNo());
		int reNo = boardDao.selectMaxReNo(groupNo, board.getComNo());
		
		if(reNo > 0) {
			
		BoardVO groupVo = boardDao.selectgroupNo(board.getComNo(), reNo);
