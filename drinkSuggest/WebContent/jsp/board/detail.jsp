...
//댓글등록
	...
	$(document).ready(
					function() {
						$('#writeMention').click( //id=writeMention인 버튼이 클릭되면 실행된다.
										function() {
											if($('#mentionContent').val().length < 1) {
												alert("내용이 없습니다")
												return
											}
	...
	$(document).ready(
			function() {
				...
				$('.reWriteMention').click( //id=writeMention인 버튼이 클릭되면 실행된다.
						function() {
							if($('#mentionContent').val().length < 1) {
								alert("내용이 없습니다")
								return
							...
							$.ajax({ //MentionWriteController로 이동
								'url' : "${ pageContext.request.contextPath }/mentionReWrite.do",  // 클라이언트가 요청을 보낼 서버의 URL 주소
								type: 'post', //요청방식
								data : {
									// 서버에서 보내줄 데이터의 타입
									//공유영역에 등록되어있는 것들
						...
						<c:if test="${not empty requestScope.mentionList }">
							<c:forEach var="mention" items="${requestScope.mentionList}">
								...
								<tr>
									...
									<!-- 본문내용 -->
									<td>
										<div style="float: left">${mention.mentionContent}</div>
									</td>
									...
									<td>
										<!-- 댓글 작성자만 수정, 삭제 가능하도록 -->
										<span>
											<button class="reWriteMention" value="${mention.mentionNo }">대댓글</button>
										</span>
										<span>
